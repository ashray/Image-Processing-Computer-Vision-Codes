%% NearestNeighbourInterpolate(ImageMat, NRowsScale, NColsScale)
% Performs enlargement of the image stored in the matrix ImageMat by
% using Nearest Neighbour Interpolation.
% 
% Same as Bilinear Interpolation but with a different impulse response
% 
...
function [ ImageMatOut ] = NearestNeighbourInterpolate(ImageMat, NRowsScale, NColsScale)
...
for i = 1:NChannels
	...
	ColumnsBiasedLeft = mod(ColsInds-1, NColsScale)<=floor(NColsScale/2.0);
	ImageTemp = ones(NRowsInitial,1)*(ColumnsBiasedLeft) ...
										.*double(ImageMat(:, LeftColInds, i)) ...                                            
			  + ones(NRowsInitial,1)*(~ColumnsBiasedLeft)...
										.*double(ImageMat(:, RightColInds, i));
    
	% This step now performs Interpolation across the rows of ImageTemp.
	RowsBiasedUp = mod(RowsInds - 1, NRowsScale)<=floor(NRowsScale/2.0);
	ImageMatOut(:,:, i) ...
				= (RowsBiasedUp)'*ones(1, NColsFinal) ...
										.*ImageTemp(UpRowInds, :) ...
				+ (~RowsBiasedUp)'*ones(1, NColsFinal) ...
